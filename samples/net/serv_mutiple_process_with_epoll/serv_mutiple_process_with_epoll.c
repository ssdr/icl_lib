...
int child_process(int listen_sock, struct epoll_event events[])
{
	int ret = epoll_dispatch(listen_sock, events);
	return ret;
}

int epoll_dispatch(int listen_sock, struct epoll_event events[])
{
	int nfds = 0, n, epollfd, conn_sock;
...
int epoll_close(int epollfd, struct epoll_event *ev)
{
	...
	for (i = 0; i < pnum; i++) {
		...
		switch (pid) {
			...
			case 0: {
						/* parent */
						printf("child:%d\n", i);
						child_process(listen_sock, events);
						/* 这里直接退出，不会进入接下来的循环工作 */
						exit(0);
